...
## Q2 · Dec 2026 – Feb 2027 — Make the site worth browsing — ✅ COMPLETE (Sep 2026)
...
- **Component QA — overlay interaction pass.** `scripts/qa-overlays.mjs` actually opens the four components the mechanical sweep never triggers — menu, dialog, tooltip, snackbar — across both viewports and themes. Found three A-severity defects — `ds-dialog`'s `actions` slot buttons had zero visual styling, `ds-tooltip`'s horizontal positions were mis-offset, and `ds-snackbar` overflowed a 360px viewport — all since fixed (see Q4's "visible A-severity defects" below). Menu and dialog's dismiss configuration held up correctly.
...
- **Visible A-severity defects cleared.** The three from the overlay pass, each traced to a root cause wider than its one component. `ds-tooltip` and `ds-menu` both landed 15px right of their target on Windows: `scrollbar-gutter: stable both-edges` on `html` offsets the `position: fixed` containing block from the viewport origin that `getBoundingClientRect()` measures from — fixed with a shared `placeFixed()` helper (`src/utils/fixed-position.js`, 7 new tests) that measures the containing block's origin with a probe, also used by `advanced-menu`'s fallback submenu. `ds-snackbar`'s 360px overflow came from `@media (max-inline-size: 600px)` — not a valid media feature, so the mobile block never applied — plus `content-box` sizing and host padding the page reset silently zeroed; the same invalid query had disabled the mobile layouts of `banner`, `bottom-sheet`, `dialog` and `side-sheet` too, all five now `max-width`. Dialog, bottom-sheet and side-sheet demos (and the settings-page example) now use `<ds-button variant="text">` in their actions slot instead of reset-stripped native buttons, and the alert dialog's actions are trailing-aligned as MD3 specifies. Verified: 13 overlays × 3 themes × 2 viewports all open, render real buttons and close; QA sweep clean; `test:a11y` 106/106; 2,114 tests pass. Three smaller findings logged to [DEFECTS.md](./DEFECTS.md) rather than fixed: dialog has no scrim, `ds-menu` measures itself while `display: none` so its flip logic never fires, and the desktop bottom sheet isn't centred.
...
3. **Visual regression** — the axe backlog and the three visible A-severity defects are both cleared (26 Sep 2026), so nothing left in this sequence blocks baselining. The one remaining A, `text-field` form association, is behavioural and doesn't affect screenshots. The open B-severity visual defects in DEFECTS.md (dark-theme state layers, dialog scrim, bottom-sheet centring, …) would be captured as they stand — fix the cheap ones first, or accept that the baselines record them and update the affected shots when they're fixed.
...
logged (see [DEFECTS.md](./DEFECTS.md)); every visible (A-severity) one is
fixed; the one A still open is behavioural (`text-field` form association). The Tier-2 pass covered all 32 remaining demo pages and found one
...
- **Accessibility trigger.** The sweep tags a11y findings (focus visibility,
  keyboard traversal) but cannot audit ARIA properly. The trigger fired: the
  axe harness was pulled forward from Q4 and built in September 2026, and
  its backlog is cleared (see Q4).
...
| **Docs drift from reality.** Structural duplication was removed by the September 2026 consolidation, but drift still recurred in figures and status claims: two re-verification passes in late September found stale test counts and bundle sizes, a roadmap line calling unfixed defects fixed, and 9 documented CSS properties/parts that didn't exist in the source. | Figures in the README's "Measured numbers" table are re-measured, never copied, and carry a date. `npm run check:docs` verifies every documented CSS property, event and part against `src/`. Re-verify claims at each roadmap review rather than trusting the previous one. |
| **A wrong demo page has nothing to catch it.** Consolidation made `docs/components/{name}.html` the sole source of API truth; there is no second copy to disagree with it. | Tests cover the behaviour the page describes; `check:docs` covers the names it documents; the QA workstream checked every page's examples; visual regression will lock the rendering in. |
| **Inventing work to fill the slack.** The plan budgeted ~200h for the year; most of it landed in September 2026, leaving ~26h of real work. The temptation is new components or tooling — both explicit non-goals. | Remaining work is the tables in Q3 and Q4 plus DEFECTS.md. When those are done, the project is done: stop, rather than extend. Re-plan only via the revisit triggers below. |
