...
| Remaining | Est. |
| --- | --- |
| **Fix the 30 axe findings still open in [DEFECTS.md](./DEFECTS.md)** (down from 38 — cluster 1 done, see above), then flip the harness's CI step from non-blocking to blocking. Four remaining shared root causes: (2) `nested-interactive` — `checkbox`/`chip`/`form`, ~1h; (3) `scrollable-region-focusable` — `carousel`/`scrollbar`/`split-button`/`text-wrapper`, ~1h if it really is one shared missing-`tabindex` pattern; (4) `aria-required-parent` — `navigation-bar`/`navigation-rail`, ~0.5h; (5) five remaining one-offs with no shared cause (`navigation-drawer`, `progress-indicator`, `carousel`'s second finding, `virtual-scroll`'s two, and three separate `color-contrast` issues), ~1.5h. | 4h |
...
logged (see [DEFECTS.md](./DEFECTS.md)); the visible-severity ones are
fixed. The Tier-2 pass covered all 32 remaining demo pages and found one
