...
2. Higher risk, because it touches component source directly: **fixing the axe findings** in DEFECTS.md (see also the form-association, package-export, and source-layout defects turned up since), then the **docs-site accessibility audit**. Both done — 106/106 checks passing as of 22 Sep 2026 (see above).
3. **Visual regression** — the QA/axe backlog this was waiting on is now fully clear, so nothing left in this sequence blocks it. Baselining screenshots on a known-good state is the next item.
